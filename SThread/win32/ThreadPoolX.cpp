...
int CThreadPoolX::Open(int nThreadCount, unsigned long nThreadIndexStart)
{
	...
	{
		return eThreadPoolXError_Failed;
	}
	...
	return eThreadPoolXError_Success;
}
...
int CThreadPoolX::ThreadProcessFunc(void* pArgs)
{
	...
	while (1)
	{
		...
		{	
			...
			{
				startTime = stopTime;
				printf("CThreadPoolX::ThreadProcessFunc process tasks %d in 1 second thread %d.\r\n", nCount, pThreadInfo->nThreadId);
				nCount = 0;
			}
