 ...
 DWORD WINAPI  CThread::ThreadPro(void *pParam)
{
	...
	p->m_pEventCreate->Set();
	if (p)
	{
		p->Run();
	}
	p->m_state = eThreadState_Stop;
	p->m_pEventExit->Set();
	//ExitThread(0);
	_endthreadex(0);
	return 0;
...
int SThread::CThread::Create( ThreadProcEx thrProcEx, void *pData, int nStackSize/*=1024*1024*/ )
{
	if (!m_pThreadData)
	{//已经创建在运行
		return -2;
...
unsigned int SThread::CThread::GetThreadId()
{
	return m_pThreadData->m_dwThreadID;
}

int SThread::CThread::Run()
{
	int nRt = 0;
#ifdef WIN32
	if (m_thrProc)
...
bool SThread::CThread::IsRunning()
{
	if (m_pThreadData->m_hThread == INVALID_HANDLE_VALUE)
		return false;
