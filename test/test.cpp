...
class TaskOne:public CRunnableX
{
	public:
		void Run();
	protected:
...
void TestThreadPoolX()
{
	TaskOne *pRunnableX = NULL;
	CThreadPoolX *pThreadPoolX = NULL;

	pThreadPoolX = new CThreadPoolX;
	if (NULL == pThreadPoolX)
	{
		return;
	}

	pThreadPoolX->Open(5, 1000);
	pThreadPoolX->Start();

	int i = 1000;
	while (1)
	{
		pRunnableX = new TaskOne();
		pThreadPoolX->AddTask(pRunnableX, i);
		i++;
		if (i == 1005)
		{
			i = 1000;
